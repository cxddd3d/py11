...
def plot_learning_curves(record_dict, sample_step=500,x_tick_step=5000):
    """
    ...
    for idx, item in enumerate(train_df.columns):
        ...
        axs[idx].grid()
        axs[idx].legend()
        x_data = range(0, train_df.index[-1], x_tick_step)  # 每隔x_tick_step步标出一个点
        axs[idx].set_xticks(x_data)
        if x_tick_step < 1000:
            axs[idx].set_xticklabels(x_data)  # 直接显示步数
        else:
