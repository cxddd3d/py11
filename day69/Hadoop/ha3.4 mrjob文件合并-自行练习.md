...
02 user2
03 user3
uid orderid order_price   不能放在源文件中
01   01     80
01   02     90
02   03    82
02   04    95
...
class UserOrderJoin(MRJob):
    ...
    def mapper(self, _, line):
        ...
        if len(fields) == 2:
            ...
            user_id = fields[0]
            user_name = fields[1]
            yield  user_id,[source,user_name] # 01 [A,user1] 02 [A,user2]
        elif len(fields) == 3:
            # order data
            source ='B'
            user_id = fields[0]
    ...
    def reducer(self,user_id,values):
        ...
        :param user_id:
        :param values:01 [A,user1] 02 [A,user2]   01 ['B',01,80]['B',02,90]
        :return:
        '''
...
下面是将最初的文件，变为如下结果，实现对两个数据表进行join操作，显示效果为每个用户所下订单的订单总量和累计消费金额
...
class UserOrderJoin(MRJob):
    ...
    def reducer(self,user_id,values):
        '''
        统计每个用户的订单数量和累计消费金额,user_id相同的资料会走到同一个reducer
        :param user_id:
        :param values:
        :return:
        '''
