...
class GridWorld_v4(object):
    ...
    def step(self, action):
        ...
        此方法根据当前状态和输入的动作，计算下一个状态、奖励和终止标志。
        
        Args:
            action (int): 要执行的动作编号，范围从 0 到 4。
        
        Returns:
            tuple: 包含下一个状态、奖励、终止标志和额外信息（此处固定为 0）的元组。
        """
        # 调用 getScore 方法，根据当前状态和动作获取奖励和下一个状态
        ...
        # 计算下一个状态的坐标
        nxtx, nxty = nextState // self.columns, nextState % self.columns
        # 如果下一个状态是目标区域，将终止标志设置为 1，表示环境已终止
        if self.scoreMap[nxtx][nxty] == self.score:
            self.terminal = 1

        # 返回下一个状态、奖励、终止标志和额外信息（此处固定为 0）
