...
def main():
    ...
    if len(sys.argv) == 3:
        ...
        # 如果没有传递命令行参数，则从配置文件中读取
        with open("./web_server.conf") as f:
            conf_info = eval(f.read())
        # 从配置文件中获取端口和框架信息
        if 'port' in conf_info and 'framework' in conf_info:
            port = conf_info['port']
            web_frame_module_app_name = conf_info['framework']
        else:
            print("配置文件中缺少port或framework配置项")
            return
    # eval是我们可以把字符串直接变为字典的一个操作
    with open("./web_server.conf") as f:
        conf_info = eval(f.read())
    # 设置静态资源访问的路径
