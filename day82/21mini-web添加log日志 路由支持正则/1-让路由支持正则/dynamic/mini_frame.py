...
def index():
    ...
    # 创建Connection连接
    conn = connect(host='192.168.19.128', port=3306, user='root', password='123', database='stock_db', charset='utf8')
    # 获得Cursor对象
    cs = conn.cursor()
    ...
    for line_info in stock_infos:
        html += tr_template % (
            ...
            line_info[7],
            line_info[1]) #把每个股票的股票代码，替换给systemidvaule
...
def application(env, start_response):
    ...
    try:
        ...
        # return func()
        for url_reg, func in URL_FUNC_DICT.items():#items遍历字典，获取键值对
            # {
            #   r"/index.html":index,
            #   r"/center.html":center,
            #   r"/add/\d+\.html":add_focus
            # }
            ret = re.match(url_reg, file_name) #match从开头开始匹配
            if ret:
                return func()
        else:
